...
#%% info
mouse_id = "191172"
ms_mat_path = r"D:\miniscope data 2\Results_191172\ms.mat"
context_orders=["A","B","B","A","B","A","A","B","A","B","A1","B1"]
context_angles=["90","90","135","135","90","90","45","45","90","90","90","90"]
behave_dir = os.path.join(r"\\10.10.46.135\data_archive\qiushou\miniscope\2019*" , mouse_id)
...
ms = ms_load['ms']
acceptedPool = ms_load['acceptedPool']
#ms = loadmat(ms_mat_path)['ms']
dff = ms['dff'] #
#S_dff=ms['S_dff']# S_dff is deconvolved dff
sigraw1 = ms['sigraw']  # estimate.c
sigraw2 = np.transpose( sigraw1);
sigraw = sigraw2[ acceptedPool-1][:]
del sigraw1, sigraw2

sigdeconvolved1 = ms['sigdeconvolved'] # estimate.s
sigdeconvolved2 = np.transpose( sigdeconvolved1)
sigdeconvolved = sigdeconvolved2[ acceptedPool-1]
del sigdeconvolved1, sigdeconvolved2
...
#%% Load the Ca2+ traces with detected events only
CaTraces = scipy.io.loadmat (ms_mat_path)['CaTraces']
tracelen = np.size( CaTraces[0][0]) # trace length
#traces = np.empty( ( ms['numNeurons'], tracelen ) ) # array for Ca traces of all neurons
traces = np.empty( ( np.size( acceptedPool), tracelen ) ) # array for Ca traces of all neurons
traces[:] = np.nan # initialize with nan
ii = 1;
for n in acceptedPool: # only take traces for accepted ROI
    traces[ii-1][:tracelen] = CaTraces[n-1][0]       
    ii = ii + 1;
del ms
...
for i,temp in enumerate(ms_ts,1):
    ...
    if i != len(ms_ts):
        ...
        starts.append(ends[-1])
        print('trace and ms_ts[',starts[i-1],ends[i-1],']constructed as DataFrame')
        for j in range(1,len(sigraw)+1):
            block[str(j)]=sigraw[j-1,starts[i-1]:ends[i-1]]
        
    else:
        block = pd.DataFrame()
        print('trace and ms_ts[',starts[i-1],'end',']constructed as DataFrame')
        for j in range(1,len(sigraw)+1):
            block[str(j)]=sigraw[j-1,starts[i-1]:]
    block['ms_ts']=temp
    msblocks.append(block)
    del i,temp,j
print("sigraw and ms_ts have been constructed as DataFrame. ")   

# For traces with detected Ca2+ events
msblocks2 = [] 
starts = [0]
ends=[]
len_ms_ts = []
for i,temp in enumerate(ms_ts,1):
    ...
    if i != len(ms_ts):
        ...
        starts.append(ends[-1])
        print('trace and ms_ts[',starts[i-1],ends[i-1],']constructed as DataFrame')
        for j in range(1,len(traces)+1):
            block[str(j)]=traces[j-1,starts[i-1]:ends[i-1]]
        
    else:
        block = pd.DataFrame()
        print('trace and ms_ts[',starts[i-1],'end',']constructed as DataFrame')
        for j in range(1,len(traces)+1):
            block[str(j)]=traces[j-1,starts[i-1]:]
    block['ms_ts']=temp
    msblocks2.append(block)
    del i,temp,j
print("detected Ca events and ms_ts have been constructed as DataFrame. ")   
...
for blockname, msblock2,aligned_behaveblock in zip(blocknames,msblocks2,aligned_behaveblocks):
    in_context  = aligned_behaveblock['in_context']
...
#%% Save <result>    
savemat(r"D:\miniscope data 2\Results_191172\191172_in_context.mat",result)
