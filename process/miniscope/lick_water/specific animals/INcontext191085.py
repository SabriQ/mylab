...
mouseinfo = MouseInfo(mouse_info_path=r"Z:\QiuShou\mouse_info\191085_info.txt")
...
behave_trackfiles = [i for i in glob.glob(behave_trackfiledir) if '204246' not in i]
behave_trackfiles.sort(key=sort_key)

behave_timestampdir = os.path.join(behave_dir,"*_ts.txt")
behave_timestamps = [i for i in glob.glob(behave_timestampdir) if '204246' not in i]
behave_timestamps.sort(key=sort_key)

behave_logffiledir = os.path.join(behave_dir,"*log.csv")
behave_logfiles = [i for i in glob.glob(behave_logffiledir) if '204246' not in i]
behave_logfiles.sort(key=sort_key)

behave_videodir = os.path.join(behave_dir,"*_labeled.mp4")
behave_videos = [i for i in glob.glob(behave_videodir) if '204246' not in i]
